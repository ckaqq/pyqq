...
function getP(a, b, c)
{
    ...
    function getEncryption(password, salt, vcode, isMd5) {
        ...
        var  s2 = md5(h1 + salt);
        var  rsaH1 = $.RSA.rsa_encrypt(h1);
        var  rsaH1Len = (rsaH1.length / 2).toString(16);
        var  hexVcode = r.TEA.strToBytes(vcode.toUpperCase(), true);
        ...
        r.TEA.initkey(s2);
        var saltPwd = r.TEA.enAsBase64(rsaH1Len + rsaH1 + r.TEA.strToBytes(salt) + vcodeLen + hexVcode);
        r.TEA.initkey("");
        __monitor(488358, 1);
